...
# 웹서버의 api 목록들 생성 (식당에서의 메뉴판을 생성)

# 데코레이션 -> @함수
# 내비게이션 함수
# 특정 주소와 함수를 연결
# 주소 -> base.url(127.0.0.1:5000) + sub_url(상대 주소) -> 이 주소로 요청이 들어왔을 때 함수를 호출한다.
...
def index():
    ...
    # template 폴더 안에 html 문서를 불러와서 되돌려준다. ( 이 행동들은 render_template()이 한다. )
    return render_template("index.html")
    # render_template에서는 {%=변수명%}는 python에서의 변수를 담겠다.
    # {{python code}} code를 인식하여 조건에 맞는 경우에만 html 추가
    #                   반복문을 이용해서 html를 반복적으로 추가


# 127.0.0.1:5000/second 주소로 요청이 들어왔을때
@app.route('/second')
def second():
    return "Second Page"
# 함수의 이름은 중복되면 안된다.
...
def login():
    ...
    # get 방식으로 보낸 데이터를 확인
    print(request.args)
    # request.args가 args 형태 -> 각각의 key 값으로 데이터를 추출
