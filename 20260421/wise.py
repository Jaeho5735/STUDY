...
while True:
    # 유저가 데이터를 입력한다.
    input_code = input('종목 코드를 입력하시오 (입력값 종료시 ENTER)')
    # 종목코드는 길이가 6
    if len(input_code) == 6:
...
for code in code_list:
    ...
    # read_html을 이용해서 table 데이터를 크롤링 -> 4번째 표를 선택
    df = pd.read_html(base_url + code)[3]

    # 데이터 프레임을 csv 파일로 저장 (파일의 이름은 종목코드, csv)
    # 데이터 프레임 타입에서 csv 타입으로 변경 -> 타입이 변경될 때 사용되는 키워드 (to)
